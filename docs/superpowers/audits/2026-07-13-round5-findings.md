...
| id | song/config | symptom | severity | root-cause hypothesis | status |
|---|---|---|---|---|---|
| P10 | guitar-loneliness-word (production path: any song using translation attach) | **Adjacent translations swapped on a correctly-ordered pair.** Sheet rows 42/43 (`出せない状態で叫んだよ` / `なんかになりたい`, 2nd chorus) display each other's EN: "I want to become something" / "I screamed without being able to let it out". Verified stage-by-stage: after `smartAttachSecondLanguage` the translations are correct (fixture files are line-aligned, `lyrics.ja.txt`/`lyrics.en.txt` rows 43–44); `fixAdjacentTranslationOrder` then swaps them. Induces 4 wrong pairs not in pairing-truth: 出せ→something, 状態→want, なんか→without, なり→let. First-occurrence rows 14/15 (different neighbors) are unaffected. | med (wrong translation text shown for 2 lines + wrong word colors) | `adjacentTranslationsSwapped` (`src/ai-pipeline/translationOrder.ts:46-58`): `lineGlossAffinity` is normalized by EN word count, so the very short `なんかになりたい` row (few alignable tokens, few EN words) produces unstable affinities and the swap margin (`+0.12`, `×1.2`) is cleared spuriously — 出す's broad JMdict gloss likely matches "something/become" in the other row's EN. | **fixed (CLASS-P1 round)** — instrumented affinities refined the hypothesis: on rows 43/44 direct affinity is 0+0 and the swap "wins" on a SINGLE spurious JMdict hit on ONE side only (状態→"want", cross0=0.25; cross1=0, so the なりたい row itself has no preference), which clears both aggregate margins whenever direct evidence is zero. A genuine clause inversion leaves evidence on BOTH rows (Veil true positive with real tokens+JMdict: cross0=1.00, cross1=0.33, both direct 0). Fix: the gate now also requires each row to individually prefer the swapped translation (`cross0 > direct0 && cross1 > direct1`) on top of the existing aggregate margins. After: rows 43/44 keep their correct translations and correct pairs (なんか→something, なり→become, たい→want, 出せ→let, 叫ん→screamed); all 4 induced wrong pairs gone; Veil true positive still detected; all 7 LRC configs byte-identical. Known measurement artifact: guitar-loneliness-word `pair_wrong` 7→8 — the pre-existing known-residual truth pair 状態→without (P11 noise floor, matched per occurrence by line text) now also fires on row 43 because that row finally has the correct translation containing "without"; documented carve-out in `corpus-pairing.test.ts` (`ALLOWED_MEASUREMENT_ARTIFACTS`), to be cleared at the Task 5 baseline ratchet. Test: real-token+JMdict cases in `tests/ai-pipeline/translationOrder.test.ts`. |
...
1. **CLASS-P1 "translation-order swap misfire on short lines"** — members: P10
   ...
   defects) per the user-impact ordering. **Fixed** — see P10 status: the swap
   gate now requires bidirectional per-row evidence, blocking one-sided
   single-gloss-hit swaps while keeping true clause inversions detected.
...
|---|---|---|---|---|
| 1 | CLASS-T1 mixed word-mode EN pass no-op (fixed: segment-granularity EN pass, `d3e3b21`) | timing | A1 (fixed) | high |
...
| 4 | CLASS-P1 translation-order swap misfire | gloss/translation | P10 (fixed) | med |
