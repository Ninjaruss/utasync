...
import { backfillLateStartsToAcousticOnset } from '../../src/lyrics/leadingEdgeAnchor'
...
describe('backfillLateStartsToAcousticOnset', () => {
  ...
  it('snaps a late start back to the vocal onset (stem), endTime preserved', () => {
    const lines = mk(8)
    const out = backfillLateStartsToAcousticOnset(lines, spans(lines), dipOnsetVoiced(6, 'stem'))
  ...
  it('does NOT snap when there is no pre-onset dip (activity all voiced)', () => {
    ...
    const out = backfillLateStartsToAcousticOnset(lines, spans(lines), loud)
  ...
  it('does NOT snap on a raw mix when the acoustic onset disagrees with the lexical onset', () => {
    const out = backfillLateStartsToAcousticOnset(mk(8), spans(mk(8)), dipOnset7('mix'))
    expect(out[0].startTime).toBe(8)
  })

  it('snaps on a stem even when the acoustic onset disagrees with the lexical onset (stem decisive)', () => {
    const out = backfillLateStartsToAcousticOnset(mk(8), spans(mk(8)), dipOnset7('stem'))
  ...
  it('DOES snap on a raw mix when the acoustic onset agrees with the lexical onset', () => {
    const out = backfillLateStartsToAcousticOnset(mk(8), spans(mk(8)), dipOnsetVoiced(6, 'mix'))
    expect(out[0].startTime).toBeLessThanOrEqual(6.1)
  })
  ...
  it('does NOT snap a poorly-anchored line (coverage below floor)', () => {
    ...
    const out = backfillLateStartsToAcousticOnset(lines, computeLineMatchedSpans(lines.map((l) => l.original), sanitizeTranscript(partial)), dipOnsetVoiced(6, 'stem'))
    expect(out[0].startTime).toBe(8)
  })
  ...
  it('trims the previous line end to avoid overlap when snapping', () => {
    ...
    const out = backfillLateStartsToAcousticOnset(lines, computeLineMatchedSpans(lines.map((l) => l.original), sanitizeTranscript(two)), dipOnsetVoiced(6.5, 'stem'))
  ...
  it('does not snap if trimming the previous line would squash it below MIN_HIGHLIGHT', () => {
    ...
    const out = backfillLateStartsToAcousticOnset(lines, computeLineMatchedSpans(lines.map((l) => l.original), sanitizeTranscript(two)), dipOnsetVoiced(6.9, 'stem'))
  ...
  it('never moves a start across the previous line', () => {
    ...
    const out = backfillLateStartsToAcousticOnset(lines, computeLineMatchedSpans(lines.map((l) => l.original), sanitizeTranscript(two)), dipOnsetVoiced(6, 'stem'))
